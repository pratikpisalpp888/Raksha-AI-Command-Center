...
export function Header({ agent, onLogout }) {
  ...
  return (
    <motion.header
      className="relative bg-[rgba(3,8,16,0.97)] backdrop-blur-xl border-b border-[rgba(255,107,0,0.2)] flex-shrink-0 z-50"
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between px-4 lg:px-6 gap-4">
        ...
        <div className="flex items-center gap-3 lg:gap-5 flex-shrink-0">
          ...
          {/* Status Badge */}
          <div className="flex items-center gap-2 rounded-full border border-[rgba(0,230,118,0.25)] bg-[rgba(0,230,118,0.08)] px-3 py-1.5 hidden lg:flex">
            <motion.span
          ...
          {/* Agent Info */}
          {agent && (
            <div className="flex items-center gap-3 pl-3 ml-2 border-l border-[rgba(255,107,0,0.2)]">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-bold text-white leading-tight">{agent.name}</p>
                <p className="text-[9px] text-[#FF9933] font-mono tracking-widest">{agent.role}</p>
              </div>
              <div className="w-8 h-8 rounded-full border border-[#FF9933] bg-[#FF9933]/10 flex items-center justify-center text-[#FF9933] font-bold text-xs shadow-[0_0_10px_rgba(255,107,0,0.2)]">
                {agent.avatar}
              </div>
            </div>
          )}
          ...
          {onLogout && (
            <button 
              onClick={onLogout}
              className="p-2 ml-1 rounded-lg text-[#7B8FA8] hover:text-[#FF2D55] hover:bg-[#FF2D55]/10 transition-all"
              title="Secure Logout"
            >
              <LogOut size={18} />
            </button>
